		...
		<view class="content" @touchstart="hideLong">
			<html2canvas ref="html2canvas" :domId="domId" @renderFinish="renderFinish">
				<view id="poster">
					...
					<!-- 文章内容模块 -->
					<article-module :pageData="pageData" @comment='clickComment' @selfzan='clickGood'>
					...
					<!-- 文章评论模块 -->
					<view class="comment">
						<comment-module @showSelect="showSelect" :avatar="pageData.article.avatar" :pageData='pageData'
							:self-good='selfGood' />
					</view>
					<!-- 长按展开模块 -->
					<longselect-module @cancel="longSelectHidden = false" @replyCom="replyCom" @renderImg="preImg" @deleteCom="deleteCom" :coordinate='coordinate' :hidden="longSelectHidden" />
					<!-- input模块 -->
					<input-module @blur="blurInput" :reply="reply" :focus='isFocus' :isFixed="isFixed" @inputMsg='getComment' />
				</view>
			</html2canvas>
		</view>
	</view>
