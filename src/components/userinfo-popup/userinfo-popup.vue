	...
	<popup ref="userpopup" type="fadeInMiddle" @hide="close">
		<view class="userinfo_popup">
			...
			<view class="content">
				...
				<view class="randomImg" @click="randomImg">
					随机头像
				</view>
			</view>
			<view class="content">
				...
				<view class="content_right">
					<input class="popup_input" type="text" v-model="username" placeholder="请填写发表者昵称" />
				</view>
				<view class="randomImg" @click="randomName">
					随机昵称
				</view>
			</view>
			<picker mode='date' value='0' fields='day' @change='getDate'>
				<edit-item title='发表日期' icon='date' arrow :right-text='date.date' :finish='status.item1' />
			</picker>
			<picker mode='time' @change='getTime'>
				<edit-item title='发表时间' icon='time' arrow :right-text='date.time' :finish='status.item2' />
			</picker>

			<view class="popup_btn" @tap="sendInfo">
				发表
			</view>
		</view>
	</popup>
