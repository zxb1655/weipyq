	...
	<view class="weui-uploader__bd">
		...
		<view class="weui-uploader__files" id="uploaderFiles">
			<block v-if='files.length == 1'>
				<image :src="files[0]" mode="widthFix" class="onepyqPic" @load="imageLoad"
					:style="{'width':firPicWidth,'height':firPicHeight}"></image>
			</block>
	...
	export default {
		...
		methods: {
			...
			imageLoad(e) {
				...
				this.firPicWidth = width + 'rpx'
				this.firPicHeight = height + 'rpx'
			},
			getPicture() {
				// detail对象，提供给事件监听函数
